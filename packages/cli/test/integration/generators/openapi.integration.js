...
const specPath = path.join(__dirname, '../../fixtures/openapi/uspto.yaml');
...
describe('openapi-generator specific files', () => {
  ...
  it('generates all the proper files', async () => {
    ...
    assert.fileContent(metadataController, 'listDataSets(): any {');
    assert.fileContent(
      metadataController,
    ...
    assert.fileContent(
      metadataController,
      `listSearchableFields(@param({name: 'dataset', in: 'path'}) dataset: ` +
        `string, @param({name: 'version', in: 'path'}) version: string): any {`,
    );
